...
# Example usage
# 학습 데이터 경로
original_data_dir ='D:\\ChanWooKim\\dataset' #Target_Data './Compete_Dataset_Refined/'
# 변환된 데이터가 저장될 경로.
new_data_dir ='D:\\ChanWooKim\\dataset\\Compete_COCO' #New Dir './Compete_COCO/'
#이미지 형태의 마스크를 polygon으로 변환하여 저장하게 되는데 이 과정에서 mask의 형태가 손실될 수 있다는 경고 존재.
#학습 Train원본을 저장하기로 함.
#split_ratio는 검증 데이터셋을 분할하는 비율이나 함수에서는 실질적으로 쓰이지 않음.
